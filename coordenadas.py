...
kml_file = str(input("Archivo KML de ingreso(sin comillas): "))
inicio=int(input("Ingrese inicio de progresiva: "))
...
loni,lati,_=coordinates_array[0]
d=0
d_falta=0
coordenadas_finales=[]
...
with open("salida.kml","w") as f:
  ...
  f.write('''  <Folder>
    <name>postes</name>\n''')
  f.write('    <Placemark>\n')
  f.write('      <name>'+ str(inicio) +'</name>\n')
  f.write('''      <styleUrl>#msn_triangle0</styleUrl>
  <Point>\n''')
  f.write('        <coordinates>' + str(loni) + ',' + str(lati) + '      </coordinates>\n')
  f.write('      </Point>\n')
  f.write('    </Placemark>\n')
  for coordenadas in coordenadas_finales:
    ...
    if (100*i) % 1000==0:
      f.write('    <Placemark>\n')
      f.write('      <name>'+ str(inicio+i/10)+str(000)+'</name>\n')
      f.write('''      <styleUrl>#msn_triangle0</styleUrl>
      <Point>\n''')
...
print("Se ha generado un archivo KML (mismo directorio de .py)\ncon el nombre de Salida con UTM 18 Zona Sur")
print("####################### FIN ###########################")